...
\begin{frame}{Back to the title}
  ...
  {\centering
    \begin{tikzpicture}[scale=\scalefilter, every node/.style={transform shape}]
      \node (A) [input, fill=yellow] {Visual measurements};
      \node (dummy1) [dummy, below=2mm of A] {};
      \node (C) [input,below=2mm of dummy1, fill=yellow] {Inertial measurements};
      \node<3-> (D) [method,right=of dummy1] {Fusion};
      \node<4-> (E) [input, right=of D, fill=\nodestatecolor] {State (Absolute scale)};
      \draw<3->[arr] (A.east) -- (D.175);
      \draw<3->[arr] (C.east) -- (D.185);
  ...
  \begin{figure}[h!]
    \centering
      \begin{subfigure}[b]{0.47\textwidth}
        ...
        \onslide<4->{\caption{Symmetry induced by the strong weight of the gravity}}
      \end{subfigure}
      \onslide<5->{
      \begin{subfigure}[b]{0.47\textwidth}
